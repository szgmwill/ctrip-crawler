...
QUNAR_BASE = Config.BASE_URL['qunar_ow']
...
def get_flights(iter_panel):
    ITER_XPATH = ".//div[@class='c1'][1]//div[@class='a_model']/span[1]"
    iters = iter_panel.xpath(ITER_XPATH)
    iter_details = []
    for i in iters:
        iter_details.append(i.text)
...
def init_files(dir_path):
    filename1 = dir_path + '/oneway_qunar_quotes.csv'
    ...
    filename2 = dir_path + '/oneway_qunar_fails.csv'
...
def qunar_scraper_oneway(query_lst, dir_path):
